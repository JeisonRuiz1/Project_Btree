...
public class ArbolNArio<T> {
	...
	public Nodo<T> searchPersonToNode(T info){
		Nodo<T> sNodo = null;
		for (Nodo<T> hijo : raiz.getHijos()) {
			if ((sNodo = searchPersonToNode(hijo.getDato())) != null) {
				if ()
				return sNodo;
			}
		}
    ...
    private void recorrerNiveles(Nodo<T> nodo, int nivel, List<T> valores) {
       // iterar los hijos del nodo
        for (Nodo<T> n : nodo.getHijos()) {
            // si ya se lleg� al nivel 0 agregar los valores a la lista
            if (nivel == 0)
